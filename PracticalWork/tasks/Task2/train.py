...
earlyStopping = keras.callbacks.EarlyStopping( 
    ...
    restore_best_weights=True # no fim de cada época é guardado na memória RAM o melhor modelo (c melhor accuracy). Logo, no fim das épocas todas, caso não tenha feito early stopping, o Keras restaura para a memória RAM o modelo com melhor accuracy, de forma a 'model.save(MODEL_PATH)' salvar esse modelo(p.ex: epoca: 195, val_accuracy: 0.69) e não o último que tinha sido lido(p.ex: epoca: 200, val_accuracy: 0.61)    
)

# parametros de treino
model.compile(
    optimizer=keras.optimizers.Adam(learning_rate=0.005533),
...
model.save("bestTeste.keras")
with open("historyTeste.pkl", "wb") as f:
    pickle.dump(history.history, f)

print("\nModelo guardado em: best.keras")
print("Histórico guardado em: history.pkl")