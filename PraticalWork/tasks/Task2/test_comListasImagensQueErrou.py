...
def ds_and_names(path):
    ...
    # ---------------------- LISTAR IMAGENS QUE ERROU ----------------------
    file_paths = d.file_paths  # caminhos completos das imagens na mesma ordem do dataset
    # ---------------------- LISTAR IMAGENS QUE ERROU ----------------------
    ...
    return d, names, file_paths

# carregar dataset
test_ds, class_names, file_paths = ds_and_names("../../dataset/test")
model = keras.models.load_model("final_model_gwo.keras")
...
# --- listar imagens em que o modelo errou ---

# criar dicionário: classe_real → lista de erros dessa classe
errors_by_class = {name: [] for name in class_names}

for i, (yp, yt) in enumerate(zip(y_pred, y_true)):
    if yp != yt:
        real_class = class_names[yt]
        pred_class = class_names[yp]
        errors_by_class[real_class].append((file_paths[i], pred_class))

print("\n================ IMAGENS INCORRETAS (AGRUPADAS POR CLASSE REAL) ================")

total_erros = 0

for classe in class_names:
    erros = errors_by_class[classe]
    print(f"\n--- {classe} ---")
    
    if len(erros) == 0:
        print("Nenhum erro nesta classe.")
        continue
    
    for path, pred in erros:
        print(f"Imagem: {path} | Correta: {classe} | Predita: {pred}")
    
    print(f"Total de erros desta classe: {len(erros)}")
