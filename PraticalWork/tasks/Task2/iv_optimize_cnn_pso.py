...
# transforma os valores vindos do algoritmo PSO (entre 0.0 e 1.0 (limites definidos em lb (low bound) e ub(upper bound))) em hiperparâmetros reais e utilizáveis pela CNN
...
def avaliar_modelo(params_0_1): # em 'params_0_1' estão os valores entre 0 e 1 criados e enviados pelo algoritmo PSO
    try:
    ...
    try:
        model = build_model_from_params(p) # cria o modelo com a arquitetura CNN ja c os valores transformados vindos do PSO
        
        earlyStopping = keras.callbacks.EarlyStopping( # callback que vai guardando na memoria RAM o melhor modelo
            monitor="val_loss", # controla pela loss
            patience=7, # nº de épocas sem melhorar até parar
        ...
        return 1.0 - accuracy # Como fitness é a metrica que o PSO tenta minimizar e a metrica que se está a usar no treino é a accuracy (que se tende a maximizar), tem que se fazer 1-aac de forma a obter o valor minimo (minimizar)
...
pop_size = 10 # 10 particulas, neste caso aves/peixes   
iterations = 10 # 10 iteracoes

w = 0.5  # Inércia
c1 = 1.5 # Cognitivo (pessoal)
c2 = 1.5 # Social (global)

# executa o otimizador do Particle Swarm Optimization (PSO)
print("--- OTIMIZAÇÃO DE HIPERPARÂMETROS (PASSO 1/2) ---")
print(f"Algoritmo: PSO | Dim: {dim} | Pop: {pop_size} | Iter: {iterations}")
...
print(f"A iniciar PSO...")
start_pso = time.time() # para cronometrar o tempo da otimizacao
pso(pop_size, avaliar_modelo, lb, ub, dim, iterations, w, c1, c2) # otimizacao
end_pso = time.time() # para terminar o cronometro

print("\n" + "=" * 50)
print(f"OTIMIZAÇÃO PSO COMPLETA em {(end_pso - start_pso) / 3600:.2f} horas")
...
if best_params_pso:
    ...
    MODEL_SAVE_PATH = "final_model_pso.keras"
    ...
    with open("history_pso.pkl", "wb") as f:
        pickle.dump(history.history, f)
        
    print("\nModelo final guardado em: final_model_pso.keras") 
    print("Histórico guardado em: history_pso.pkl")
    print("=" * 50)
